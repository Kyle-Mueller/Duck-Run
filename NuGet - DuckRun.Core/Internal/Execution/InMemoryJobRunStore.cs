...
internal sealed class InMemoryJobRunStore : IJobRunStore
{
    ...
    private readonly int _maxPerJob;

    public InMemoryJobRunStore(DuckRunOptions options)
    {
        _maxPerJob = options.RunsRetainedPerJob;
    }
    ...
    public Task<JobRun?> GetAsync(Guid runId, CancellationToken ct) =>
        Task.FromResult(_byId.TryGetValue(runId, out var run) ? run : null);
